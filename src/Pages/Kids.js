  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const WomensProducts = response.data.products.filter(
        ...
        <div className="col-md-9">
          ...
          <div className="row">
            {products.map((product) => (
              <div className="women-card-container col-md-4 mb-4"  key={product.id} onClick={() => navigate(`/product/${product.id}`)}style={{ cursor: "pointer" }} >
                  ...
                  <div className="women-card-body text-center">
                    ...
                    <div className="d-flex justify-content-center gap-3">
                      <FaHeart className="women-heart-icon text-danger"  style={{ cursor: "pointer" }}
                        onClick={(e) => {  e.stopPropagation();   alert(`Liked ${product.title}`);}}/>
